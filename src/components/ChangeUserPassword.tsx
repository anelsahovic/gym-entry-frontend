...
export default function ChangeUserPassword({ user, className, text }: Props) {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errorMessage, setErrorMessage] = useState('');

  const form = useForm<ChangeUserPasswordBody>({
    resolver: zodResolver(ChangeUserPasswordSchema),
  });

  const onSubmit = async (values: ChangeUserPasswordBody) => {
    try {
      setIsSubmitting(true);
      setErrorMessage('');

      const response = await updateUserPassword(user.id, values);

      if (response.status === 200) {
        toast.success('User password changed successfully');
        setOpen(false);
      } else {
        toast.error('Failed changing user password.');
      }
    } catch (error) {
